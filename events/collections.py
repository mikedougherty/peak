...
class SubSet(trellis.Set):
    ...
    base = trellis.make(trellis.Set, writable=True, name='base')
...
class Hub(trellis.Component):
    ...
    _queries = trellis.cellcache(lambda self,key:(), resetting_to=(), name='_queries')
    _inputs = trellis.make(trellis.Pipe, name='_inputs')
    _index = trellis.make(dict, name='_index')     # {(pos,value):{rule:1}}}
...
class Observing(trellis.Component):
    ...
    keys = trellis.make(trellis.Set, name='keys')
    ...
    watched_values = trellis.maintain(watched_values, initially=({}, {}), name='watched_values')
    ...
    def changes(self):
        ...
        return changes
    changes = trellis.maintain(changes, resetting_to={}, name='changes')
...
class SortedSet(trellis.Component):
    ...
    data    = trellis.make(trellis.Set, writable=True, name='data')
