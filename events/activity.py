...
class EventLoop(trellis.Component, context.Service):
    ...
    _call_queue = trellis.make(list, writable=True, name='_call_queue')
    _next_time = trellis.compute(lambda self: Time.next_event_time(True), name='_next_time')
...
class Time(trellis.Component, context.Service):
    ...
    _schedule = trellis.make(lambda self: [Max], writable=True, name='_schedule')
    _events = trellis.cellcache(lambda self, key: False, name='_events')
    ...
    def _tick(self):
        if self.auto_update:
            ...
            return tick
        return self._tick
    _tick = trellis.maintain(_tick, initially=EPOCH._when, name='_tick')
